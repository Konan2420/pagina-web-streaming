-- La migración anterior eliminó la colisión de variables PL/pgSQL. Esta fija
-- además el contrato de la CTE: las expresiones UNION ALL no tienen nombres
-- estables por sí mismas, por lo que se declaran explícitamente.
-- No se modifican datos, tablas ni políticas RLS.
...
BEGIN
  ...
  RETURN QUERY
  WITH base_rows (
    id, source, seller_id, business_client_id, profile_id, producto_id, producto_nombre,
    image_url, account_reference, client_name, client_phone, avatar_url, brand,
    created_at, expires_at, business_status, estado, is_renewable, auto_renew,
    auto_renew_at, unit_cost_pen, sale_price_pen, profit_pen
  ) AS (
