-- Garantiza que la bonificación configurada por administración sea global.
-- La recarga puede pertenecer a un cliente, proveedor o distribuidor; el rol
-- nunca se utiliza como filtro de elegibilidad. Solo administración puede
-- aprobar la operación mediante approve_recarga.
...
BEGIN
  ...
  -- No se consulta user_roles: cualquier beneficiario autenticado es elegible.
...
COMMENT ON FUNCTION public.approve_recarga(uuid, numeric) IS
  'Aprueba recargas de clientes, proveedores y distribuidores aplicando el porcentaje global vigente.';
