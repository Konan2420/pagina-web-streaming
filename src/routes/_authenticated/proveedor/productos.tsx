    ...
    try {
      ...
      toast.success(
        product.is_catalog_available
          ? "Producto marcado fuera de servicio."
          : "Producto habilitado para compra.",
      );
      await queryClient.invalidateQueries({ queryKey: ["provider-products"] });
    ...
    >
      <div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <p className="text-sm text-white/50">
          {products.length} producto{products.length === 1 ? "" : "s"} propio
          {products.length === 1 ? "" : "s"}
        </p>
        <button
          onClick={openCreate}
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-xs font-black uppercase tracking-wide text-white transition hover:brightness-110"
        >
      ...
      <div className="overflow-hidden rounded-2xl border border-white/8 bg-white/[0.025]">
        <div className="cmd-table-scroll">
          <table className="w-full min-w-[650px] whitespace-nowrap text-left text-sm">
            ...
            <tbody className="divide-y divide-white/6">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-5 py-16 text-center text-white/35">
                    <Boxes className="mx-auto mb-3 h-6 w-6" />
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-1">
                        <button
                          ...
                          className="rounded-lg p-2 text-white/50 transition hover:bg-white/7 hover:text-emerald-300 disabled:opacity-40"
                          aria-label={
                            product.is_catalog_available
                              ? `Marcar ${product.name} fuera de servicio`
                              : `Habilitar ${product.name} para compra`
                          }
                          title={
                            product.is_catalog_available
                        ...
                        <button
                          ...
                          className="rounded-lg p-2 text-white/50 transition hover:bg-white/7 hover:text-white"
                        ...
                        <button
                          ...
                          className="rounded-lg p-2 text-white/50 transition hover:bg-red-500/10 hover:text-red-300 disabled:opacity-40"
      ...
      {formOpen && (
        <div className="fixed inset-0 z-[90] grid place-items-center bg-black/75 p-4 backdrop-blur-sm">
          <form
            onSubmit={handleSave}
            className="flex max-h-[calc(100dvh-2rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-ink shadow-2xl"
          >
            <header className="flex items-center justify-between border-b border-white/8 p-5">
              <div>
                ...
                <p className="mt-1 text-xs text-white/45">
                  Se guardará como borrador para aprobación.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-y-auto p-5 sm:grid-cols-2">
              ...
              <Field label="Tipo de entrega">
                <select
                  ...
                  defaultValue={editing?.credential_template ?? "account"}
                  className={fieldClass}
                >
                  <option value="account">Cuenta y contraseña</option>
                  <option value="account_2fa">Cuenta con 2FA y recuperación</option>
                  <option value="redeem_code">Código de canje</option>
                  <option value="access_link">Enlace de acceso</option>
              ...
              <Field label="ID de icono">
                <input
                  name="icon_id"
                  defaultValue={editing?.icon_id || ""}
                  className={fieldClass}
                />
              </Field>
              ...
              <label className="flex items-end gap-2 pb-2 text-sm text-white/70">
                <input
                  name="is_renewable"
                  type="checkbox"
                  defaultChecked={editing?.is_renewable ?? true}
              ...
              <Field label="Descripción detallada" className="sm:col-span-2">
                <textarea
                  ...
                  rows={4}
                  defaultValue={editing?.descripcion_larga || ""}
                  className={`${fieldClass} resize-y`}
                />
              </Field>
            </div>
            <footer className="flex justify-end gap-3 border-t border-white/8 p-5">
              <button
                type="button"
                onClick={() => setFormOpen(false)}
                className="rounded-xl border border-white/12 px-4 py-2.5 text-xs font-bold text-white/70 hover:bg-white/5"
              >
                Cancelar
              </button>
              <button
                disabled={saving}
                type="submit"
                className="rounded-xl bg-primary px-4 py-2.5 text-xs font-black uppercase tracking-wide text-white disabled:opacity-60"
              >
...
function Field({
  ...
  return (
    <label
      className={`grid gap-1.5 text-xs font-bold uppercase tracking-wide text-white/45 ${className}`}
    >
      <span>
        {label}
        {required ? " *" : ""}
