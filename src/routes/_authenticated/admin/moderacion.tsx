...
function statusBadge(status: BanStatus) {
  const labels: Record<BanStatus, string> = {
    active: "Activo",
    expired: "Expirado",
    revoked: "Revocado",
  };
  ...
  return (
    <span
      className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide ${colors[status]}`}
    >
      {labels[status]}
    </span>
  );
...
function ModerationPage() {
  ...
  const filtered = bans.filter((ban) => {
    ...
    const matchesSearch =
      !term ||
      [ban.user.name, ban.user.email, ban.ip_address || "", ban.reason].some((value) =>
    ...
    return (
      ...
      afterStart &&
      beforeEnd &&
      (status === "all" || ban.status === status) &&
      (role === "all" || ban.user.role === role)
    );
  });
    ...
    <AdminLayout title="Moderación" subtitle="Suspensiones de cuentas e historial de auditoría">
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        ...
        <Summary
          ...
          label="Suspensiones permanentes"
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_11rem_11rem_10rem_10rem]">
          <label className="relative block">
            ...
            <input
              ...
              placeholder="Buscar por usuario, correo, IP o motivo…"
              className="h-10 w-full rounded-lg border border-input bg-background pl-10 pr-3 text-sm text-foreground outline-none focus:border-primary"
            />
          </label>
          <select
            value={status}
            onChange={(event) => setStatus(event.target.value as "all" | BanStatus)}
          ...
          <label className="sr-only" aria-label="Desde">
            <input
              type="date"
              value={fromDate}
          ...
          <label className="sr-only" aria-label="Hasta">
            <input
              type="date"
              value={toDate}
          ...
          <table className="min-w-[980px] w-full text-left text-sm">
            <thead className="border-b border-border bg-muted/40 text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-3">Administró</th>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-14 text-center text-sm text-muted-foreground">
                    No se encontraron baneos para los filtros aplicados.
                  </td>
                </tr>
              ) : (
                filtered.map((ban) => (
                  <tr key={ban.id} className="align-top transition-colors hover:bg-muted/30">
                    <td className="px-4 py-3">
                      <p className="font-semibold text-foreground">{ban.user.name}</p>
                      <p className="mt-0.5 text-xs text-muted-foreground">
                        {ban.user.email} · {ban.user.role}
                      </p>
                    </td>
                    <td className="max-w-64 px-4 py-3 text-sm text-foreground">
                      <p className="line-clamp-2">{ban.reason}</p>
                      {ban.revocation_reason && (
                        <p className="mt-1 text-xs text-emerald-300">
                          Revocación: {ban.revocation_reason}
                        </p>
                      )}
                    </td>
                    <td className="px-4 py-3 font-mono text-xs text-muted-foreground">
                      {ban.ip_address || "No registrada"}
                    </td>
                    <td className="px-4 py-3 text-xs leading-5 text-muted-foreground">
                      <p>Inicio: {dateLabel(ban.starts_at)}</p>
                      <p>Fin: {dateLabel(ban.ends_at)}</p>
                    </td>
                    <td className="px-4 py-3">{statusBadge(ban.status)}</td>
                    <td className="px-4 py-3 text-xs text-muted-foreground">
                      <p>{ban.bannedBy}</p>
                      {ban.revokedBy && (
                        <p className="mt-1 text-emerald-300">Revocado por {ban.revokedBy}</p>
                      )}
                    </td>
                    <td className="px-4 py-3 text-right">
                      {ban.status === "active" && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => setRevokeTarget(ban)}
                          className="border-emerald-400/30 text-emerald-300 hover:bg-emerald-400/10 hover:text-emerald-200"
                        >
                          <Undo2 className="size-3.5" /> Revocar
                        </Button>
                      )}
                    </td>
                  </tr>
      ...
      <Dialog open={Boolean(revokeTarget)} onOpenChange={closeRevoke}>
        <DialogContent className="bg-card sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Revocar baneo</DialogTitle>
            <DialogDescription>
              Restaurará el acceso de {revokeTarget?.user.name}. Si el baneo añadió una IP
              vinculada, también se revocará esa restricción asociada.
            </DialogDescription>
          </DialogHeader>
          <form className="space-y-4" onSubmit={handleRevoke}>
            <label className="block space-y-1.5 text-sm font-medium text-foreground">
              ...
              <textarea
                ...
                maxLength={1000}
                className="min-h-20 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary"
              />
            </label>
            <label className="flex items-start gap-3 rounded-lg border border-emerald-400/25 bg-emerald-400/5 p-3 text-sm text-foreground">
              <input
                required
                type="checkbox"
              ...
              />
              <span>Confirmo que deseo levantar esta suspensión.</span>
            </label>
            <DialogFooter>
              <Button
                type="button"
              ...
              <Button
                type="submit"
                disabled={revoking || !revokeConfirmed}
                className="bg-emerald-600 text-white hover:bg-emerald-500"
              >
                {revoking && <Loader2 className="size-4 animate-spin" />} Revocar baneo
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
