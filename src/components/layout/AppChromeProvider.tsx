...
import { AppChromeContext, type AppChromeContextValue, type ColorMode } from "./AppChromeContext";

const commands = [
  { label: "Ir al catálogo", description: "Productos y servicios disponibles", href: "/" },
  {
    label: "Redes Sociales",
    description: "Generador y órdenes de servicios",
    href: "/redes-sociales",
  },
        ...
        <DialogContent className="max-w-lg border-border bg-card p-0 text-foreground">
          <DialogHeader className="border-b border-border px-5 pb-4 pt-5">
            <DialogTitle className="flex items-center gap-2 text-base">
              <Command className="h-4 w-4 text-primary" /> Buscador global
            </DialogTitle>
            ...
            <div className="mt-3 max-h-72 space-y-1 overflow-y-auto">
              {visibleCommands.map((command) => (
                <button
                  ...
                  className="flex w-full items-start gap-3 rounded-lg px-3 py-3 text-left transition-colors hover:bg-white/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                >
