  ...
  return (
    <aside className="xl:sticky xl:top-4 xl:self-start">
      <div className="mb-3 flex items-center justify-between gap-3">
        <h3 className="font-display text-base font-bold text-white">Visualizador en vivo</h3>
        <a
          href={publicHref}
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-white/65 transition hover:text-white"
        >
          <Eye className="h-3.5 w-3.5" /> Ver tienda pública
        </a>
      </div>
      <div
        data-cmd-fixed-contrast
        className="overflow-hidden rounded-[1.25rem] border border-white/10 shadow-2xl shadow-black/30"
        style={{ backgroundColor: template.surface, color: template.text }}
      >
        <div
          className="relative h-28 overflow-hidden sm:h-36"
          style={{
            background: `linear-gradient(135deg, ${template.surface}, ${template.accent}66)`,
          }}
        >
          {settings.bannerUrl && (
            <img
        ...
        <div className="relative px-4 pb-4 pt-0">
          ...
          <div className="mt-2 flex items-start justify-between gap-3">
            ...
            <span className="grid h-8 w-8 place-items-center rounded-md border border-white/10 bg-black/15 text-white/75">
              <Moon className="h-3.5 w-3.5" />
              <Sun className="sr-only" />
            </span>
          </div>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {socialLinks.map(({ href, label, icon: Icon }) => (
                <a
                  ...
                  rel="noreferrer"
                  aria-label={label}
                  className="grid h-6 w-6 place-items-center rounded-full border border-white/10 bg-black/15 text-white/75 transition hover:text-white"
                >
          ...
          <label className="relative mt-4 block">
            ...
            <input
              readOnly
              placeholder="Buscar cuentas, juegos..."
              className="h-9 w-full rounded-md border border-white/10 bg-black/15 pl-9 pr-3 text-[11px] text-white outline-none placeholder:text-white/35"
            />
          </label>
          <div className="mt-3 grid grid-cols-2 gap-2">
            {products.slice(0, 4).map((product) => (
              <article
                key={product.id}
                className="min-h-24 overflow-hidden rounded-md border border-white/10 bg-black/15 p-2.5"
              >
                <div
                  className="h-8 rounded bg-white/[0.05]"
                  style={
                    product.imageUrl
                      ? {
                ...
                />
                <p className="mt-2 line-clamp-1 text-[10px] font-bold text-white">{product.name}</p>
                <p className="mt-0.5 text-[10px] font-bold" style={{ color: template.accentSoft }}>
                  {money(product.price)}
                </p>
              </article>
            ))}
          ...
          {products.length === 0 && (
            <div className="mt-3 rounded-md border border-dashed border-white/15 p-5 text-center text-[10px] text-white/45">
              Publica productos para verlos aquí.
            </div>
          )}
        </div>
      </div>
      <p className="mt-2 text-[10px] leading-relaxed text-white/40">
        *Esta es una previsualización aproximada. La tienda pública reflejará los colores exactos de
