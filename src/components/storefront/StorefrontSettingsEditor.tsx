        ...
        <div className="grid min-h-0 flex-1 xl:grid-cols-[minmax(0,1fr)_27rem]">
          <div className="cmd-dark-scrollbar min-h-0 overflow-y-auto px-5 py-5 sm:px-7">
            ...
            <section className="mt-7 border-t border-border/70 pt-6"><h3 className="text-sm font-bold text-white">Identidad Visual</h3><p className="mt-1 text-xs text-white/45">Personaliza tu foto de perfil y portada de tienda.</p>
              <div className="mt-4 grid gap-5 sm:grid-cols-[9rem_minmax(0,1fr)]"><div><p className="mb-2 text-xs font-semibold text-foreground">Avatar</p><div className="cmd-media-frame grid h-32 w-32 place-items-center overflow-hidden rounded-full border-4 border-card text-2xl font-black text-foreground">{form.logoUrl ? <img src={form.logoUrl} alt="Avatar de tienda" className="h-full w-full object-cover" /> : form.displayName.slice(0, 2).toUpperCase()}</div><label className="mt-3 inline-flex h-11 cursor-pointer items-center gap-2 rounded-md border border-border bg-card px-3 text-xs font-semibold text-foreground transition hover:border-primary/60 sm:h-9">{uploading === "logo" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />} Subir<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={!ownerId || uploading !== null} onChange={(event) => void handleImage("logo", event.target.files?.[0])} /></label></div>
                <div><p className="text-xs font-semibold text-foreground">Marco de Avatar <span className="font-normal text-muted-foreground">(Opcional)</span></p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Destaca en la tienda con un marco animado.</p><div className="mt-3 flex flex-wrap gap-2">{avatarFrames.map((frame) => <button key={frame.name} type="button" onClick={() => update({ avatarFrameKey: frame.key })} className={form.avatarFrameKey === frame.key ? "cmd-active-subtle h-11 rounded-md border px-2.5 text-xs font-bold sm:h-8" : "h-11 rounded-md border border-border bg-card px-2.5 text-xs font-semibold text-foreground transition hover:border-primary/60 sm:h-8"}>{frame.name}</button>)}</div>{form.avatarFrameKey && <button type="button" onClick={() => update({ avatarFrameKey: null })} className="mt-3 h-11 px-2 text-xs font-semibold text-red-300 transition hover:text-red-200 sm:h-auto">Quitar Marco</button>}</div></div>
              <div className="mt-5"><p className="text-xs font-semibold text-foreground">Portada de Tienda <span className="font-normal text-muted-foreground">(Opcional)</span></p><p className="mt-1 text-xs text-muted-foreground">Aparecerá en la parte superior de tu tienda. Medidas recomendadas: 1024×256 px.</p><div className="cmd-media-frame mt-3 overflow-hidden rounded-lg border border-border">{form.bannerUrl ? <img src={form.bannerUrl} alt="Vista previa de portada" className="h-28 w-full object-cover" /> : <div className="grid h-28 place-items-center text-xs text-muted-foreground">Sin portada seleccionada</div>}</div><label className="mt-3 inline-flex h-11 cursor-pointer items-center gap-2 rounded-md border border-border bg-card px-3 text-xs font-semibold text-foreground transition hover:border-primary/60 sm:h-9">{uploading === "banner" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ImageUp className="h-3.5 w-3.5" />} Subir portada<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={!ownerId || uploading !== null} onChange={(event) => void handleImage("banner", event.target.files?.[0])} /></label><p className="mt-2 text-[10px] text-muted-foreground">JPG, PNG o WebP; máximo 5 MB.</p></div>
              <div className="mt-5 grid gap-3 sm:grid-cols-2"><Input label="Nombre público" value={form.displayName} onChange={(value) => update({ displayName: value })} /><Input label="Enlace público" value={form.storeSlug} onChange={(value) => update({ storeSlug: value })} /><label className="sm:col-span-2 grid gap-1.5 text-xs font-semibold text-foreground">Descripción<textarea value={form.description} onChange={(event) => update({ description: event.target.value })} className="min-h-20 rounded-lg border border-border bg-background p-3 text-sm text-foreground outline-none focus:border-primary/70" /></label></div>
              ...
              <label className="mt-3 grid gap-1.5 text-xs font-semibold text-foreground">Disponibilidad<select value={form.availabilityMode} onChange={(event) => update({ availabilityMode: event.target.value as "manual" | "schedule" })} className="h-10 rounded-lg border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-primary/70"><option value="manual">Manual</option><option value="schedule">Por horario</option></select></label>
            ...
            <section className="mt-7 border-t border-border/70 pt-6"><h3 className="text-sm font-bold text-foreground">Plantilla Base</h3><div className="mt-3 grid gap-2 sm:grid-cols-2">{storefrontTemplates.map((template) => <button key={template.key} type="button" onClick={() => update({ templateKey: template.key })} className={form.templateKey === template.key ? "cmd-active-subtle flex min-h-14 items-center gap-2 rounded-lg border px-3 text-left" : "flex min-h-14 items-center gap-2 rounded-lg border border-border bg-card px-3 text-left transition hover:border-primary/60"}><span className="h-5 w-5 shrink-0 rounded-full" style={{ background: `linear-gradient(135deg, ${template.accentSoft}, ${template.accent})` }} /> <span className="min-w-0 flex-1"><span className="flex items-center gap-1 text-xs font-bold text-foreground">{template.premium && <Crown className="h-3 w-3 text-amber-300" />}{template.name}</span><span className="mt-0.5 block text-[10px] text-muted-foreground">{template.description}</span></span><span className={form.templateKey === template.key ? "h-3 w-3 rounded-full border-4 border-primary bg-card" : "h-3 w-3 rounded-full border border-border"} /></button>)}</div><p className="mt-3 text-xs text-muted-foreground">Configura el estilo visual fundamental de tu tienda pública.</p></section>
          ...
          <div className="cmd-dark-scrollbar min-h-0 overflow-y-auto border-t border-border bg-muted/40 px-5 py-5 sm:px-7 xl:border-l xl:border-t-0">
...
function Input({ label, value, onChange, placeholder, type = "text" }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; type?: string }) {
  return <label className="grid gap-1.5 text-xs font-semibold text-foreground">{label}<input type={type} value={value} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="h-11 rounded-lg border border-border bg-background px-3 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/70 sm:h-10" /></label>;
}
...
function SwitchRow({ label, description, checked, onChange }: { label: string; description: string; checked: boolean; onChange: (value: boolean) => void }) {
  return <label className="flex items-center justify-between gap-3 rounded-lg border border-border bg-muted/45 px-3 py-2.5 text-xs"><span><span className="block font-semibold text-foreground">{label}</span><span className="mt-0.5 block text-[10px] text-muted-foreground">{description}</span></span><Switch checked={checked} onCheckedChange={onChange} /></label>;
}
