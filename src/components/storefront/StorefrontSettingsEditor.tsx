...
} from "@/components/storefront/StorefrontLivePreview";
...
export function StorefrontSettingsEditor({
  ...
  const handleImage = async (kind: "banner" | "logo", file?: File) => {
    ...
    try {
      ...
      toast.success(
        kind === "banner"
          ? "Portada cargada. Publícala para hacerla visible."
          : "Avatar cargado. Publícalo para hacerlo visible.",
      );
    } catch (error) {
  ...
  const publish = async () => {
    ...
    await onPublish({
      ...
      avatarFrameKey: form.avatarFrameKey,
      ...
      <DialogContent className="flex max-h-[calc(100vh-1.5rem)] max-w-[1180px] flex-col overflow-hidden border-border bg-[#121722] p-0 text-white sm:max-h-[calc(100vh-3rem)]">
        <DialogHeader className="border-b border-border px-5 py-4 text-center sm:px-7">
          <DialogTitle className="font-display text-xl font-bold sm:text-2xl">
            {isAdministrativeEditing
              ? `Configuración de la tienda de ${ownerName}`
              : "Configuración de tu Tienda"}
          </DialogTitle>
          {isAdministrativeEditing && (
            <p className="mt-1 text-xs text-amber-200">
              Estás editando una tienda ajena como administrador. La publicación quedará auditada.
            </p>
          )}
        </DialogHeader>
          ...
          <div className="cmd-dark-scrollbar min-h-0 overflow-y-auto px-5 py-5 sm:px-7">
            <section>
              <div className="flex items-end justify-between gap-3">
                <div>
                  <h3 className="text-sm font-bold text-white">Ajustes de Diseño</h3>
                  <p className="mt-1 text-xs text-white/45">
                    Elige la plantilla que mejor se adapte a tu marca.
                  </p>
                </div>
              ...
              <button
                ...
                className="mt-4 inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-red-accent px-4 text-xs font-bold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60 sm:h-10"
              >
                <Save className="h-4 w-4" />
                {saving
                  ? "Publicando cambios…"
                  : isAdministrativeEditing
                    ? "Publicar cambios en esta tienda"
                    : "Publicar cambios en mi tienda"}
              </button>
            </section>

            <section className="mt-7 border-t border-border/70 pt-6">
              <h3 className="text-sm font-bold text-white">Identidad Visual</h3>
              <p className="mt-1 text-xs text-white/45">
                Personaliza tu foto de perfil y portada de tienda.
              </p>
              <div className="mt-4 grid gap-5 sm:grid-cols-[9rem_minmax(0,1fr)]">
                <div>
                  <p className="mb-2 text-xs font-semibold text-foreground">Avatar</p>
                  <div className="cmd-media-frame grid h-32 w-32 place-items-center overflow-hidden rounded-full border-4 border-card text-2xl font-black text-foreground">
                    {form.logoUrl ? (
                      <img
                        src={form.logoUrl}
                  ...
                  <label className="mt-3 inline-flex h-11 cursor-pointer items-center gap-2 rounded-md border border-border bg-card px-3 text-xs font-semibold text-foreground transition hover:border-primary/60 sm:h-9">
                    {uploading === "logo" ? (
              ...
              <div className="mt-5">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Aparecerá en la parte superior de tu tienda. Medidas recomendadas: 1024×256 px.
                </p>
                <div className="cmd-media-frame mt-3 overflow-hidden rounded-lg border border-border">
                  {form.bannerUrl ? (
                    <img
                      src={form.bannerUrl}
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                ...
                <Input
                  ...
                  onChange={(value) => update({ storeSlug: value })}
                />
                <label className="sm:col-span-2 grid gap-1.5 text-xs font-semibold text-foreground">
                  Descripción
                  <textarea
                    value={form.description}
                    onChange={(event) => update({ description: event.target.value })}
                    className="min-h-20 rounded-lg border border-border bg-background p-3 text-sm text-foreground outline-none focus:border-primary/70"
                  />
                </label>
              </div>
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                <SwitchRow
                  ...
                  description="Permite que los clientes la visiten."
                  checked={form.isPublic}
                  onChange={(value) => update({ isPublic: value })}
                />
                <SwitchRow
                  label="Disponible ahora"
                  description="Indica si aceptas pedidos actualmente."
              ...
              <label className="mt-3 grid gap-1.5 text-xs font-semibold text-foreground">
                ...
                <select
                  ...
                  onChange={(event) =>
                    update({ availabilityMode: event.target.value as "manual" | "schedule" })
                  }
                  className="h-10 rounded-lg border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-primary/70"
                >
              ...
              {form.availabilityMode === "schedule" && (
                <div className="mt-3 grid grid-cols-2 gap-3">
                  <Input
                    ...
                    type="time"
                    value={form.opensAt}
                    onChange={(value) => update({ opensAt: value })}
                  />
                  <Input
                    label="Hasta"
                    type="time"
            ...
            <section className="mt-7 border-t border-border/70 pt-6">
              <h3 className="text-sm font-bold text-foreground">Plantilla Base</h3>
                  ...
                  <button
                    ...
                    className={
                      form.templateKey === template.key
                        ? "cmd-active-subtle flex min-h-14 items-center gap-2 rounded-lg border px-3 text-left"
                        : "flex min-h-14 items-center gap-2 rounded-lg border border-border bg-card px-3 text-left transition hover:border-primary/60"
                    }
                  >
                    <span
                      className="h-5 w-5 shrink-0 rounded-full"
                      style={{
                        background: `linear-gradient(135deg, ${template.accentSoft}, ${template.accent})`,
                      }}
                    />{" "}
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center gap-1 text-xs font-bold text-foreground">
                        {template.premium && <Crown className="h-3 w-3 text-amber-300" />}
                        {template.name}
                      </span>
                    ...
                    <span
                      className={
                        form.templateKey === template.key
                          ? "h-3 w-3 rounded-full border-4 border-primary bg-card"
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                Configura el estilo visual fundamental de tu tienda pública.
              </p>
            </section>

            <section className="mt-7 border-t border-border/70 pb-2 pt-6">
              <h3 className="text-sm font-bold text-white">Redes Sociales</h3>
              <p className="mt-1 text-xs leading-relaxed text-white/45">
                Tus clientes podrán contactarte directamente. Solo se mostrarán los íconos de las
                redes que completes.
              </p>
  ...
  return (
    <label className="grid gap-1.5 text-xs font-semibold text-foreground">
      ...
      <input
        ...
        className="h-11 rounded-lg border border-border bg-background px-3 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/70 sm:h-10"
      />
    </label>
  );
}
...
function SwitchRow({
  ...
  return (
    <label className="flex items-center justify-between gap-3 rounded-lg border border-border bg-muted/45 px-3 py-2.5 text-xs">
      <span>
