        ...
        <div className="grid min-h-0 flex-1 xl:grid-cols-[minmax(0,1fr)_27rem]">
          <div className="cmd-dark-scrollbar min-h-0 overflow-y-auto px-5 py-5 sm:px-7">
            <section>
              ...
              <button type="button" disabled={saving || uploading !== null} onClick={() => void publish()} className="mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-red-accent px-4 text-xs font-bold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"><Save className="h-4 w-4" />{saving ? "Publicando cambios…" : isAdministrativeEditing ? "Publicar cambios en esta tienda" : "Publicar cambios en mi tienda"}</button>
            </section>
            ...
            <section className="mt-7 border-t border-border/70 pt-6"><h3 className="text-sm font-bold text-white">Identidad Visual</h3><p className="mt-1 text-xs text-white/45">Personaliza tu foto de perfil y portada de tienda.</p>
              <div className="mt-4 grid gap-5 sm:grid-cols-[9rem_minmax(0,1fr)]"><div><p className="mb-2 text-xs font-semibold text-white/80">Avatar</p><div className="grid h-32 w-32 place-items-center overflow-hidden rounded-full border-4 border-card bg-background text-2xl font-black text-white">{form.logoUrl ? <img src={form.logoUrl} alt="Avatar de tienda" className="h-full w-full object-cover" /> : form.displayName.slice(0, 2).toUpperCase()}</div><label className="mt-3 inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border border-white/15 px-3 text-xs font-semibold text-white transition hover:border-white/35">{uploading === "logo" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Upload className="h-3.5 w-3.5" />} Subir<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={!ownerId || uploading !== null} onChange={(event) => void handleImage("logo", event.target.files?.[0])} /></label></div>
                <div><p className="text-xs font-semibold text-white/80">Marco de Avatar <span className="font-normal text-white/40">(Opcional)</span></p><p className="mt-1 text-xs leading-relaxed text-white/45">Destaca en la tienda con un marco animado.</p><div className="mt-3 flex flex-wrap gap-2">{avatarFrames.map((frame) => <button key={frame.name} type="button" onClick={() => update({ avatarFrameKey: frame.key })} className={form.avatarFrameKey === frame.key ? "h-8 rounded-md border border-primary bg-primary/10 px-2.5 text-xs font-bold text-primary" : "h-8 rounded-md border border-border bg-background px-2.5 text-xs font-semibold text-white/65 transition hover:border-white/30"}>{frame.name}</button>)}</div>{form.avatarFrameKey && <button type="button" onClick={() => update({ avatarFrameKey: null })} className="mt-3 text-xs font-semibold text-red-300 transition hover:text-red-200">Quitar Marco</button>}</div></div>
              <div className="mt-5"><p className="text-xs font-semibold text-white/80">Portada de Tienda <span className="font-normal text-white/40">(Opcional)</span></p><p className="mt-1 text-xs text-white/45">Aparecerá en la parte superior de tu tienda. Medidas recomendadas: 1024×256 px.</p><div className="mt-3 overflow-hidden rounded-lg border border-border bg-background">{form.bannerUrl ? <img src={form.bannerUrl} alt="Vista previa de portada" className="h-28 w-full object-cover" /> : <div className="grid h-28 place-items-center text-xs text-white/35">Sin portada seleccionada</div>}</div><label className="mt-3 inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border border-white/15 px-3 text-xs font-semibold text-white transition hover:border-white/35">{uploading === "banner" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ImageUp className="h-3.5 w-3.5" />} Subir portada<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={!ownerId || uploading !== null} onChange={(event) => void handleImage("banner", event.target.files?.[0])} /></label><p className="mt-2 text-[10px] text-white/40">JPG, PNG o WebP; máximo 5 MB.</p></div>
...
function Input({ label, value, onChange, placeholder, type = "text" }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; type?: string }) {
  return <label className="grid gap-1.5 text-xs font-semibold text-white/75">{label}<input type={type} value={value} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-sm text-white outline-none placeholder:text-white/30 focus:border-red-accent/70" /></label>;
}
