      ...
      >
        <div className={cn("p-4 md:p-6", collapsed && "md:px-3")}>
          <div className="mb-5 flex items-center justify-between gap-3 md:mb-8">
            ...
            <button
              type="button"
              onClick={onClose}
              className="grid h-11 w-11 shrink-0 place-items-center rounded-lg text-white/65 transition hover:bg-white/5 hover:text-white md:hidden"
              aria-label="Cerrar menú administrativo"
            >
              <X className="h-5 w-5" />
            </button>
          ...
          <nav className="space-y-1" aria-label="Panel administrativo">
            {items.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={cn("h-5 w-5 shrink-0", active ? "text-white" : item.accent)} />
                  <span className={collapsed ? "md:sr-only" : "truncate"}>{item.label}</span>
                </Link>
              );
            })}
          </nav>
        ...
        <div className={cn("mt-auto border-t border-white/5 p-4 md:p-6", collapsed && "md:px-3")}>
          <div className={cn("flex items-center gap-3", collapsed && "md:justify-center")}>
            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-primary/30 bg-primary/20">
              <Users className="h-5 w-5 text-primary" />
            </div>
            <div className={collapsed ? "md:sr-only" : "overflow-hidden"}>
              <p className="truncate text-sm font-semibold text-white">Admin Panel</p>
              <p className="truncate text-xs text-white/40">CMD Streaming</p>
            </div>
          </div>
        </div>
      </aside>
...
export function AdminRouteShell() {
  ...
  return (
    <AdminRouteShellContext.Provider value>
      <div className="min-h-screen bg-background text-foreground md:flex">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <AppTopbar onToggleSidebar={toggleNavigation} />
          <Outlet />
        </div>
      </div>
    </AdminRouteShellContext.Provider>
  );
}
