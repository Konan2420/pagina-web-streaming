...
function PlatformShortcutIcon({ platform }: { platform: PlatformShortcut }) {
  if (platform.iconUrl) {
    return (
      <span aria-hidden="true" className="relative grid h-10 w-10 place-items-center overflow-hidden rounded-full bg-white/[0.08] text-[10px] font-black text-white shadow-sm transition-transform duration-200 sm:h-11 sm:w-11">
        <span>{platform.fallback}</span>
        <img src={platform.iconUrl} alt="" loading="lazy" onError={(event) => { event.currentTarget.style.display = "none"; }} className="absolute inset-0 h-full w-full bg-background object-contain p-1" />
      </span>
    );
  }

  const iconId = platform.iconId ?? getPlatformIconIdByName(platform.label);
  if (iconId) return <PlatformIconMark iconId={iconId} className="h-10 w-10 transition-transform duration-200 sm:h-11 sm:w-11" />;

  return <span aria-hidden="true" className="grid h-10 w-10 place-items-center overflow-hidden rounded-full bg-white/[0.08] text-xs font-black text-white shadow-sm transition-transform duration-200 sm:h-11 sm:w-11">{platform.fallback}</span>;
}
      ...
      <div className="mx-auto max-w-[1600px] px-4 sm:px-6">
        <div className="pt-5 sm:pt-6">
          ...
          <div className="relative">
            <div ref={categoryScroll.scrollRef} className={cn("flex touch-pan-x gap-2 overflow-x-auto pb-2 scrollbar-none select-none", categoryScroll.isDragging ? "cursor-grabbing" : "cursor-grab")} role="tablist" aria-label="Categorías del catálogo" onPointerDown={categoryScroll.onPointerDown} onPointerMove={categoryScroll.onPointerMove} onPointerUp={categoryScroll.onPointerUp} onPointerCancel={categoryScroll.onPointerCancel} onClickCapture={categoryScroll.onClickCapture}>
              {categories.map((category) => {
                ...
                return <button key={category.id} type="button" role="tab" aria-selected={active} onClick={() => onCategorySelect(category.id)} className={cn("min-h-11 shrink-0 rounded-md border px-3 text-[10px] font-bold transition-[background-color,border-color,color,opacity,transform] duration-150 active:scale-[0.98] active:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-accent sm:h-8 sm:min-h-8 sm:text-[11px]", active ? "border-red-accent bg-red-accent text-white" : control)}>{category.label}</button>;
              })}
            </div>
          ...
          <div className="catalog-platform-grid mt-4">
            {visiblePlatformShortcuts.map((platform) => <button key={platform.label} type="button" onClick={() => onPlatformSelect(platform)} title={`Filtrar por ${platform.label}`} className="group relative flex h-12 w-12 items-center justify-center rounded-full border border-border bg-card p-1 transition duration-200 hover:scale-[1.04] hover:border-red-accent/70 hover:shadow-[0_0_14px_rgba(239,40,48,0.24)] active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-accent sm:h-[52px] sm:w-[52px]"><PlatformShortcutIcon platform={platform} /><span className="sr-only">Filtrar por {platform.label}</span></button>)}
          </div>
        </div>
      </div>
    </section>
