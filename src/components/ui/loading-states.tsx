...
export function GlobalLoadingBar() {
  ...
  return (
    <div
      ...
      className={cn(
        "pointer-events-none fixed inset-x-0 top-0 z-[100] h-1 overflow-hidden bg-primary/10 transition-opacity duration-200",
...
export function QueryErrorState({
  ...
  return (
    <section
      role="alert"
      className={cn(
        "rounded-xl border border-destructive/35 bg-destructive/[0.07] p-5 text-center sm:p-7",
        className,
      )}
    >
      <WifiOff className="mx-auto h-5 w-5 text-destructive" aria-hidden="true" />
      ...
      <button
        type="button"
        onClick={onRetry}
        className="mt-4 inline-flex h-9 items-center gap-2 rounded-lg border border-destructive/50 px-3 text-xs font-bold text-red-100 transition hover:bg-destructive/15 hover:text-white"
      >
