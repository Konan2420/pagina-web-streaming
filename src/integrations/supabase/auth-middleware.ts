...
async function authenticateRequest(): Promise<AuthenticatedRequestContext> {
  // Nitro follows a composable naming convention (`use*`), but this runs on
  // the server rather than in a React component. Alias it so React Hooks lint
  // does not misclassify this framework API as a React hook.
